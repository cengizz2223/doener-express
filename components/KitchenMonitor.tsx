...
export default function KitchenMonitor() {
  ...
  const handleCardDragOver = (e: React.DragEvent) => { if (e.dataTransfer.types.includes('cardid')) { e.preventDefault(); e.dataTransfer.dropEffect = 'move'; } };
  ...
  const handleRemoveEmployee = async (name: string) => {
    ...
    try {
      ...
      if (res.ok) setEmployees((prev) => prev.filter((e) => e.name !== name));
    } catch {}
  };
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="bg-zinc-900 border-b border-zinc-800 px-4 py-3">
        <div className="flex items-center justify-between mb-2.5">
          <span className="text-xs font-semibold text-zinc-500 uppercase tracking-wider">
            Mitarbeiter — auf Artikel ziehen
          </span>
          <button
            onClick={() => { setAddingEmp((v) => !v); setEmpError(''); setNewEmpName(''); }}
            className="flex items-center gap-1 text-xs bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-zinc-400 hover:text-white px-2.5 py-1 rounded-lg transition-all font-medium"
          >
            {addingEmp ? '✕ Abbrechen' : '+ Mitarbeiter'}
          </button>
        </div>
        ...
        <div className="flex items-center gap-4 flex-wrap">
          {employees.map((emp) => {
            ...
            return (
              <div key={emp.name} className="relative group">
                <div
                  draggable
                  onDragStart={(e) => handleWorkerDragStart(e, emp.name)}
                  onDragEnd={handleWorkerDragEnd}
                  className={`flex flex-col items-center gap-1.5 cursor-grab active:cursor-grabbing select-none transition-all ${isDragging ? 'opacity-40 scale-95' : 'hover:scale-105'}`}
                >
                  <div className="relative w-14 h-14 rounded-full flex items-center justify-center text-2xl font-black text-white shadow-lg"
                    style={{ backgroundColor: emp.color, boxShadow: `0 0 18px ${emp.color}50`, outline: `3px solid ${emp.color}`, outlineOffset: '3px' }}>
                    {emp.name[0]?.toUpperCase()}
                    {count > 0 && (
                      <span className="absolute -top-1 -right-1 min-w-[20px] h-5 bg-white text-zinc-900 text-xs font-black rounded-full flex items-center justify-center px-1 shadow">
                        {count}
                      </span>
                    )}
                  </div>
                  <div className="text-center">
                    <div className="text-sm font-bold leading-none" style={{ color: emp.color }}>{emp.name}</div>
                    <div className="text-xs text-zinc-600 mt-0.5 leading-none">{count > 0 ? `${count} Artikel` : 'frei'}</div>
                  </div>
                </div>
          ...
          {employees.length === 0 && !addingEmp && (
            <span className="text-sm text-zinc-600">Noch keine Mitarbeiter — oben hinzufügen</span>
          )}

          {draggingWorker && (
            <div className="ml-2 text-sm font-semibold animate-pulse" style={{ color: employees.find((e) => e.name === draggingWorker)?.color ?? '#fff' }}>
      ...
      {/* ══ HEADER: Suche + Filter ══ */}
      <div className="bg-zinc-900/90 border-b border-zinc-800 px-4 py-3 sticky top-0 z-10 backdrop-blur-sm">
        <div className="flex items-center justify-between mb-2.5">
          <h2 className="text-sm font-bold text-white flex items-center gap-2">
            <span className="w-6 h-6 bg-orange-500/10 border border-orange-500/20 rounded-lg flex items-center justify-center text-xs">🍳</span>
            Küchenmonitor
          </h2>
        ...
        <div className="flex gap-1 overflow-x-auto pb-0.5">
          {FILTER_TABS.map(({ value, label }) => (
            <button key={value} onClick={() => setFilter(value)}
              className={`flex-shrink-0 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all ${filter === value ? 'bg-orange-600 text-white' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700 hover:text-zinc-300'}`}>
